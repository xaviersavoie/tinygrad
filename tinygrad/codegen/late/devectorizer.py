...
from tinygrad.helpers import getenv, flatten, AMX, CPU_LLVM, prod
...
def split_load_store(ctx:Renderer|None, ls:UOp, idx:UOp):
  ...
  if ctx is not None and ctx.device == "DSP":
    ...
    else: lengths = [16,8,4,2] if AMX else [8,4,2] if CPU_LLVM and ls.op is Ops.LOAD and sz >= 8 else [4,2]
