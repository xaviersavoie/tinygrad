...
def hand_coded_optimizations(k:Scheduler) -> Scheduler:
  ...
  # CPU matvec: skip upcast/unroll when reduce axis is stride-1 in all buffers, let LLVM loop vectorizer handle it
  if k.ren is not None and k.ren.device == "CPU" and k.reduceop is not None and len(k.axes_of(AxisType.REDUCE)) == 1 \
    ...
    reduce_rng = k.rngs[k.axes_of(AxisType.REDUCE)[0]]
    if all(any(c is reduce_rng for c in b.src[1].get_idx().split_uop(Ops.ADD))
           for b in k.bufs if reduce_rng in b.src[1].get_idx().backward_slice):
