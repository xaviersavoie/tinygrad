...
def _gen_src(name: str, M: int, K: int, half: bool) -> str:
  ...
  def fma_line(i): return f'      a{i} = {a.fma.format(w=ldw.format(f"data1 + (m+{i})*{K} + k"), x="xv", c=f"a{i}")};'
  def store_line(i): return f'    data0[m+{i}] = {a.hsum.format(f"a{i}")};'
  NL = '\n'
  return f"""\
#include <{a.inc}>
{a.helpers}void {name}(float* restrict data0, {wtype}* restrict data1, float* restrict data2) {{
  for (int m = 0; m < {M}; m += 8) {{
    {a.vec} {', '.join(f'a{i}={a.zero}' for i in range(4))};
    {a.vec} {', '.join(f'a{i}={a.zero}' for i in range(4, 8))};
    for (int k = 0; k < {K}; k += {a.width}) {{
      {a.vec} xv = {a.ld32.format('data2 + k')};
{NL.join(fma_line(i) for i in range(8))}
    }}
{NL.join(store_line(i) for i in range(8))}
  }}
}}
"""
...
def can_use_cpu_matvec(a: Tensor, b: Tensor) -> bool:
  """Check if a @ b can use custom CPU matvec. b must be (K,M) PERMUTE view of contiguous (M,K) weight."""
  arch = _get_arch()
